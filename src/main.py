...
from arquitectura_b import ejecutar_procesos
...
def guardar_medicion_csv(resultado, archivo_csv):
    """Guarda una medición en bench/mediciones.csv usando append."""
    archivo_csv = Path(archivo_csv)
    archivo_csv.parent.mkdir(parents=True, exist_ok=True)
    existe = archivo_csv.exists() and archivo_csv.stat().st_size > 0
...
semilla,
    ...
    """
    Ejecuta una configuración 4 veces.
    La primera ejecución se descarta.
    La medición final utiliza la mediana de las
    tres ejecuciones restantes.
    """
    tiempos = []
    resultados = []

    for intento in range(4):
        resultado = correr_config(
            n_workers=n_workers,
        ...
        tiempos.append(resultado["segundos"])
        resultados.append(resultado)
    
        print(
            f"Worker {n_workers} | "
            f"ejecución {intento + 1}/4 | "
            f"tiempo: {resultado['segundos']:.4f}s"
        )

    # Se descarta SIEMPRE la primera ejecución.
    tiempos_validos = tiempos[1:]
    
    # El documento exige MEDIANA, no promedio.
    tiempo_mediano = statistics.median(tiempos_validos)
    
    # Todos los workers generan el mismo total de eventos.
    ultimo = resultados[-1]
    
    eventos = ultimo["eventos"]
    bytes_totales = ultimo["bytes"]
    ...
    resultado_final = {
        ...
        "transferencia_MB_s": transferencia_mb_s,
    }
...
if __name__ == "__main__":
    ...
    RAIZ = Path("data/raw")
    SEMILLA = 42
    ARCHIVO_CSV = Path("bench/mediciones.csv")

    resultados = []

    print("=== BENCHMARK ===")
    print(f"Eventos totales: {TOTAL_EVENTOS:,}")
    print("Configuraciones: 1, 2, 4, 8, 16 workers")
    print("Repeticiones: 4")
    print("Primera ejecución: DESCARTADA")
    print("Estadístico: MEDIANA")

    for n_workers in CONFIGS:
        resultado = medir_configuracion(
            n_workers=n_workers,
            total_eventos=TOTAL_EVENTOS,
            raiz=RAIZ,
            semilla=SEMILLA,
        ...
        assert (
            resultado["eventos_totales"] == TOTAL_EVENTOS
        ), resultado

        guardar_medicion_csv(
            resultado,
            ARCHIVO_CSV,
        )
        ...
        print(
            f"  Mediana: {resultado['tiempo']:.4f} s"
        )
        print(
            f"  Throughput: "
            f"{resultado['throughput']:,.0f} eventos/s"
        )
        print(
            f"  Transferencia: "
            f"{resultado['transferencia_MB_s']:.2f} MB/s"
        )
    ...
    for resultado in resultados:
        print(
            f"{resultado['workers']:>2} workers | "
            f"{resultado['eventos_totales']:>15,} eventos | "
